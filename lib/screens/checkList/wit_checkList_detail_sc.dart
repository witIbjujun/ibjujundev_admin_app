...
class CheckListDetail extends StatefulWidget {
  ...
  const CheckListDetail({Key? key, required this.checkInfo}) : super(key: key);
...
class CheckListDetailState extends State<CheckListDetail> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          widget.checkInfo["inspNm"],
          style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
        ),
      ),
      body: CheckListDetailView(
        inspNm: widget.checkInfo["inspNm"],
  ...
  Future<void> getCheckListByLv2() async {
    ...
    final param = jsonEncode({
      "inspId": widget.checkInfo["inspId"],
    });
  ...
  Future<void> getCheckListByLv3(String inspId) async {
    ...
    // PARAM
    final param = jsonEncode({
      "parentsInspId": widget.checkInfo["inspId"],
      "inspId": inspId,
    });
